...
```bash
stepMultiStepWorkflow$ sam build
```
...
```bash
stepMultiStepWorkflow$ sam logs -n StockCheckerFunction --stack-name stepMultiStepWorkflow --tail
```
...
```bash
stepMultiStepWorkflow$ cd functions/stock-checker
...
```bash
sam delete --stack-name stepMultiStepWorkflow
```
